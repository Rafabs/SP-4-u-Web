...
function formatFarePrice(price?: string): string {
  const parsed = Number.parseFloat(price ?? "");
  if (!Number.isFinite(parsed)) return "R$ 4,40";
...
function renderAllLines(filteredRoutes: GTFSRoute[] | null = null): void {
  ...
  const routesToRender = filteredRoutes ?? data.routes;
  ...
  data.calendar.forEach((c) => calendarMap.set(c.service_id, c));

  const fareRuleMap = new Map<string, GTFSFareRule>();
  data.fareRules.forEach((rule) => {
    if (!fareRuleMap.has(rule.route_id)) fareRuleMap.set(rule.route_id, rule);
  });
  ...
  function processChunk(): void {
    ...
    for (; currentIndex < limit; currentIndex++) {
      ...
      const fareRule = fareRuleMap.get(route.route_id);
      const fare = fareRule ? data.fares.find((f) => f.fare_id === fareRule.fare_id) : null;
      const preco = formatFarePrice(fare?.price);
      ...
      const agencyName = escapeHtml(data.agency ?? "Não identificada");
